...
imp <- read_csv('Data/Imputation-Data-Set/CNA-imputation-test.csv')
names <- colnames(imp)
names[(length(names))] <- 'labels'
colnames(imp) <- names

res <- read_csv('Data/Distribution-Data-Set/test_cna_distribution.csv')
colnames(res) <- names

ran <- read_csv('Data/Random-Data-Set/test_cna_random.csv')
colnames(ran) <- names
...
ggsave('Data/Figure-1.png', device='png',width = 12.5, height = 10, units = c("in"))
